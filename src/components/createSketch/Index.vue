<template>
     <el-row class="el-row" style="overflow: hidden;">
        ...
        <el-col class="el-col el-col-form"  :span="6">
            ...
            >
                ...
                <el-form-item :label="('canvas.xianjiqz')">
                    <el-slider v-model="style.darkCut" :min="20" :max="200" :step="1"/>
                </el-form-item>

                <el-form-item :label="('canvas.xiandzsl')">
      ...
      <div ref="arcomodalfooter" class="arco-modal-footer">
            ...
            <a-button type="primary"  @click="createBgImageDone">确认</a-button>
      </div>
  </template>
  ...
  <script setup  lang="ts">
    ...
    let label={
        jixi:('canvas.jixi'),
        jingxi: ('canvas.jingxi'),
        yiban: ('canvas.yiban'),
        shaochu: ('canvas.shaochu'),
        chaochu: ('canvas.chaochu'),
        jichu:  ('canvas.jichu'),
        fudiao: ('canvas.fudiao'),
        xiaogao: ('canvas.xiaogao')
    }
   ...
   const state = reactive<TImageCreateBgState>({
    ...
    const createBgImageDone = () => {
        ...
        canvas.toBlob(async function(blob) {
            ...
            let url= await api.upload.uploadFile(blob).catch(()=>{
